...
WHERE old.domain_id != new.domain_id
    AND new.vocabulary_id IN ('SNOMED Veterinary')
;
...
WHERE c2.vocabulary_id IS NULL
    AND c1.vocabulary_id IN ('SNOMED Veterinary')
;
...
FROM prodv5.concept c
    JOIN prodv5.concept_synonym cs
      ON c.concept_id = cs.concept_id WHERE c.vocabulary_id IN ('SNOMED Veterinary')
GROUP BY
    c.concept_code,
    c.vocabulary_id
         ),
...
WHERE a.vocabulary_id IN ('SNOMED Veterinary')
...
WITH crm AS (
    ...
    WHERE vocabulary_id_1 IN ('SNOMED Veterinary')
                                 ...
                                 AND EXISTS
                                         (SELECT 1
                                          ...
                                          WHERE concept_id_1 = o.source_concept_id
                                            ...
                                            AND invalid_reason = 'D'
                                            AND relationship_id LIKE 'Maps to%'
                                            AND concept_id_1 IN (SELECT concept_id FROM concept c WHERE c.vocabulary_id IN ('SNOMED Veterinary')))
...
LEFT JOIN concept b ON b.concept_id = concept_id_2
WHERE a.vocabulary_id IN ('SNOMED Veterinary') AND a.invalid_reason IS NULL
...
WHERE a.concept_id_2 != b.concept_id_2
    ...
    AND c.vocabulary_id IN ('SNOMED Veterinary')
