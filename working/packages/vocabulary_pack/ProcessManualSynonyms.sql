...
BEGIN
	...
	IF z > 1 THEN
		RAISE EXCEPTION 'ProcessManualSynonyms: more than one dev_schema found';
	END IF;
	...
	IF CURRENT_SCHEMA = 'devv5' THEN
		...
		IF z = 0 THEN
			RAISE EXCEPTION 'ProcessManualSynonyms: % not found', cSchemaName || '.concept_synonym_manual';
		END IF;
