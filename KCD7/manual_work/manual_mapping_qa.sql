...
********************************/
-- check presence of problems in KCD7 manual mapping - all queries should return null 
WITH kcd7_proc_and_cond
AS
(SELECT a.*,
       c.concept_class_id,
       c.domain_id
...
WHERE repl_by_name = ''
  UNION ALL
SELECT 'incorrect KCD7 icd_code' AS issue_desc,
       COUNT(icd_code)
FROM refresh_lookup_done
WHERE icd_code NOT IN (SELECT concept_code FROM concept WHERE vocabulary_id = 'KCD7')
  UNION ALL
SELECT 'incorrect KCD7 icd_name' AS issue_desc,
       COUNT(icd_name)
FROM refresh_lookup_done
WHERE icd_name NOT IN (SELECT concept_name FROM concept WHERE vocabulary_id = 'KCD7')
...
FROM (SELECT *
  ...
  JOIN concept c1 ON a1.icd_code = c1.concept_code AND c1.vocabulary_id = 'KCD7'  
  JOIN concept c2 ON a2.icd_code = c2.concept_code AND c2.vocabulary_id = 'KCD7'                         
...
FROM (SELECT *
  ...
  JOIN (SELECT *
        ...
        WHERE icd_code IN (SELECT icd_code
                         ...
                         GROUP BY icd_code
                         HAVING COUNT(1) >= 2)) a2 ON a1.icd_code = a2.icd_code
  JOIN concept c1 ON c1.concept_code = a1.icd_code AND c1.vocabulary_id = 'KCD7'
  JOIN concept c2 ON c2.concept_code = a2.icd_code AND c1.vocabulary_id = 'KCD7'
  JOIN concept_ancestor ca
    ON c1.concept_id = ca.ancestor_concept_id
   AND c2.concept_id = ca.descendant_concept_id
   AND c1.concept_id <> c2.concept_id
