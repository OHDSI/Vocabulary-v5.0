CREATE OR REPLACE FUNCTION vocabulary_download.get_snomed_veterinary (
...
$BODY$
DECLARE
pVocabularyID constant text:='SNOMED VETERINARY';
...
/*
  ...
  ALL (default), JUMP_TO_SNOMEDVET_PREPARE, JUMP_TO_SNOMEDVET_IMPORT
*/
...
BEGIN
  pVocabularyOperation:='GET_SNOMEDVET';
  ...
  if iOperation not in ('ALL', 'JUMP_TO_SNOMEDVET_PREPARE', 'JUMP_TO_SNOMEDVET_IMPORT'
  ...
  if pJumpToOperation='ALL' then
    ...
    --start downloading
    pVocabularyOperation:='GET_SNOMEDVET downloading';
    perform run_wget (
      iPath=>pVocabulary_load_path,
      iFilename=>'snomed_vet.zip',
      iDownloadLink=>pDownloadURL
    );
    perform write_log (
      iVocabularyID=>pVocabularyID,
      iSessionID=>pSession,
      iVocabulary_operation=>'GET_SNOMEDVET downloading complete',
      iVocabulary_status=>1
    );
  end if;
  
  if pJumpToOperation in ('ALL','JUMP_TO_SNOMEDVET_PREPARE') then
    pJumpToOperation:='ALL';
    --extraction
    pVocabularyOperation:='GET_SNOMEDVET prepare';
    perform get_snomedvet_prepare (
      iPath=>pVocabulary_load_path,
      iFilename=>'snomed_vet.zip'
    );
    perform write_log (
      iVocabularyID=>pVocabularyID,
      iSessionID=>pSession,
      iVocabulary_operation=>'GET_SNOMEDVET prepare complete',
      iVocabulary_status=>1
    );
  end if;
  
  if pJumpToOperation in ('ALL','JUMP_TO_SNOMEDVET_IMPORT') then
    pJumpToOperation:='ALL';
    --finally we have all input tables, we can start importing
    pVocabularyOperation:='GET_SNOMEDVET load_input_tables';
    perform sources.load_input_tables(pVocabularyID,pVocabularyNewDate,pVocabularyNewVersion);
    perform write_log (
      iVocabularyID=>pVocabularyID,
      iSessionID=>pSession,
      iVocabulary_operation=>'GET_SNOMEDVET load_input_tables complete',
      iVocabulary_status=>1
    );
  end if;
  ...
  perform write_log (
    iVocabularyID=>pVocabularyID,
    iSessionID=>pSession,
    iVocabulary_operation=>'GET_SNOMEDVET all tasks done',
