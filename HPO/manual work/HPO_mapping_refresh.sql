-- Create hpo_mapped table and pre-populate it with the resulting manual table of the previous hcpcs refresh.

DROP TABLE dev_hpo.hpo_mapped;
CREATE TABLE dev_hpo.hpo_mapped
(
...
ALTER TABLE dev_hpo.hpo_mapped ADD CONSTRAINT idx_pk_mapped UNIQUE (source_code,target_concept_code,source_vocabulary_id,target_vocabulary_id,relationship_id);
...
--9.3.4. Truncate the hpo_mapped table. Save the spreadsheet as the hpo_mapped table and upload it into the working schema.
TRUNCATE TABLE dev_hpo.hpo_mapped;

--Format after uploading
UPDATE dev_hpo.hpo_mapped SET mapping_tool = NULL WHERE mapping_tool = '';
UPDATE dev_hpo.hpo_mapped SET mapping_source = NULL WHERE mapping_source = '';
UPDATE dev_hpo.hpo_mapped SET confidence = NULL WHERE confidence = '';
UPDATE dev_hpo.hpo_mapped SET relationship_id_predicate = NULL WHERE relationship_id_predicate = '';
UPDATE dev_hpo.hpo_mapped SET cr_invalid_reason = NULL WHERE cr_invalid_reason = '';
UPDATE dev_hpo.hpo_mapped SET source_invalid_reason = NULL WHERE source_invalid_reason = '';
UPDATE dev_hpo.hpo_mapped SET mapper_id = NULL WHERE mapper_id = '';
UPDATE dev_hpo.hpo_mapped SET reviewer_id = NULL WHERE reviewer_id = '';

-- Perform any mapping checks you have set.

-- Iteratively repeat steps 9.3.2-9.3.5 if found any issues.

-- Change concept_relationship_manual table according to hpo_mapped table.
--Insert new relationships
--Update existing relationships
INSERT INTO dev_hpo.concept_relationship_manual AS mapped
    (concept_code_1,
    concept_code_2,
    vocabulary_id_1,
    vocabulary_id_2,
	...
	SELECT source_code,
           ...
           m.cr_invalid_reason
	FROM dev_hpo.hpo_mapped m
	--Only related to HPO vocabulary
	WHERE (source_vocabulary_id = 'HPO' OR target_vocabulary_id = 'HPO')
