...
WITH s0 AS (
   SELECT DISTINCT ON (m.id) m.moduleid,
	...
	FROM sources.vet_der2_ssrefset_moduledependency m
	WHERE m.active = 1
		...
		m.moduleid IN (
			'332351000009108' -- SNOMED Veterinary 
			)
	ORDER BY m.id,
...
SELECT DISTINCT ON (m.moduleid) m.moduleid,
	CASE
		WHEN m.moduleid = '332351000009108'
			THEN TO_CHAR(MAX(s0.local_version) OVER (), 'yyyy-mm-dd')
		END AS version
FROM sources.vet_der2_ssrefset_moduledependency m
LEFT JOIN s0 ON m.moduleid = s0.moduleid AND
                m.sourceeffectivetime = s0.local_version
WHERE m.moduleid IN (
			'332351000009108' -- SNOMED Veterinary edition
			)
	ORDER BY m.moduleid,
		m.effectivetime DESC;
...
-- 4. Create version of SNOMED Veterinary without concept_id, domain_id, concept_class_id, standard_concept
...
SELECT sct2.concept_name,
	...
	sct2.concept_code,
	TO_DATE(replace(substring(cast(effectivestart as text),1,8),'-',''),'YYYYMMDD') AS valid_start_date,
...
FROM (
			...
			ORDER BY c.active DESC,
				...
				CASE l.refsetid
					WHEN '332501000009101'
						THEN 1 -- SNOMED Veterinary English language reference set
					ELSE 99 
					END,
				CASE l.source_file_id
					WHEN 'VET'
						THEN 1 -- SNOMED Veterinary
					ELSE 99
					END ASC,
	...
	JOIN sources.vet_der2_crefset_language l ON l.referencedcomponentid = d.id
	) sct2
WHERE sct2.rn = 1;

ANALYZE concept_stage;

--4.1 For concepts with latest entry in vet_sct2_concept having active = 0, preserve invalid_reason and valid_end date
...
FROM (
	...
	FROM (
		SELECT DISTINCT ON (c.id) c.id,
			TO_DATE(replace(substring(cast(c.effectivetime as text),1,8),'-',''), 'YYYYMMDD') AS effectiveend,
			c.active
		FROM sources.vet_sct2_concept_full c
		ORDER BY c.id,
			TO_DATE(replace(substring(cast(c.effectivetime as text),1,8),'-',''), 'YYYYMMDD') DESC
		) s0
	WHERE s0.active = 0
	) i
...
-- have concurrent sources, so this may prevent problems and breaks nothing
						ORDER BY active DESC,
							...
							CASE f7
								...
								WHEN 'environment / location'
									THEN 52
									-- for SNOMED Veterinary
								WHEN 'animal life circumstance'
								THEN 53
								WHEN 'Includes sub-specimen'
								THEN 54
								WHEN 'Has physiologic state'
								THEN 55
								ELSE 99
								END
						) AS rnc
				FROM (
					SELECT concept_code,
						...
						rna AS rnb -- row number in sct2_desc_full
					FROM (
						SELECT c.concept_code,
							...
							ROW_NUMBER() OVER (
								...
								) rna -- row number in sct2_desc_full
	...
	SELECT concept_code,
		CASE
			...
			WHEN F7 = 'Has physiologic state'
				THEN 'Has physiol state'
			WHEN F7 = 'Includes sub-specimen'
				THEN 'Includes sub-specimen'
			ELSE 'Undefined'
...
*****/
--7. Add active synonyms from descriptions list
INSERT INTO concept_synonym_stage (
	synonym_concept_code,
	synonym_vocabulary_id,
	synonym_name,
...
FROM (
	SELECT m.conceptid,
		...
		FIRST_VALUE(m.active) OVER (
			PARTITION BY m.id ORDER BY TO_DATE(replace(substring(cast(effectiveTime as text),1,8),'-',''),'YYYYMMDD') DESC
			) AS active_status
...
--8. Fill concept_relationship_stage from SNOMED  Veterinary source
...
SELECT cs.concept_code AS concept_code_1,
	...
	'SNOMED Veterinary' AS vocabulary_id_1,
	'SNOMED Veterinary' AS vocabulary_id_2,
...
WHERE c.moduleid IN (
		...
		'332351000009108' --Veterinary edition
...
(
	SELECT DISTINCT ON (c.id) c.id AS concept_code_1,
		...
		TO_DATE(replace(substring(cast(c.effectivetime as text),1,8),'-',''), 'YYYYMMDD') AS valid_start_date,
		TO_DATE('20991231', 'YYYYMMDD') AS valid_end_date,
		NULL AS invalid_reason
	FROM sources.vet_sct2_concept_full c
	WHERE EXISTS (
...
WITH attr_rel AS (
		...
		FROM (
			...
			ORDER BY r.id,
				TO_DATE(replace(substring(cast(r.effectivetime as text),1,8),'-',''), 'YYYYMMDD') DESC,
...
SELECT DISTINCT sn.concept_code_1,
	...
	'SNOMED Veterinary',
	'SNOMED Veterinary',
	substring(sn.relationship_id,1,20),
	TO_DATE(replace(substring(cast(sn.effectivestart as text),1,8),'-',''), 'YYYYMMDD'),
...
FROM (
	SELECT sc.referencedcomponentid AS concept_code_1,
		...
		refsetid,
		ROW_NUMBER() OVER (
			PARTITION BY sc.referencedcomponentid ORDER BY TO_DATE(replace(substring(cast(sc.effectivetime as text),1,8),'-',''), 'YYYYMMDD') DESC,
		...
		ROW_NUMBER() OVER (
			...
			sc.targetcomponent,
			sc.refsetid,
			sc.moduleid ORDER BY TO_DATE(replace(substring(cast(sc.effectivetime as text),1,8),'-',''), 'YYYYMMDD') DESC
	...
	WHERE sc.refsetid IN (
			'900000000000526001', -- replaced by historical
			'900000000000523009', -- possibly equivalent to historical
			'900000000000528000', -- was a historical
			'900000000000527005', -- same as historical
			'900000000000530003' -- is a historical
			)
	) sn
WHERE EXISTS (
		SELECT 1
...
SELECT cs.concept_code AS concept_code_1,
	...
	'SNOMED Veterinary' AS vocabulary_id_1,
	'SNOMED Veterinary' AS vocabulary_id_2,
	substring(cr.relationship_id,1,20),
	cr.valid_start_date,
	(
...
CREATE UNLOGGED TABLE snomed_ancestor AS
	...
	concepts AS (
		...
		WHERE crs.invalid_reason IS NULL
			...
			AND crs.vocabulary_id_1 = 'SNOMED Veterinary'
			AND crs.vocabulary_id_2 = 'SNOMED Veterinary'
		)
	...
	JOIN concept_stage cs1 ON cs1.concept_code = hc.root_ancestor_concept_code
		AND cs1.vocabulary_id = 'SNOMED Veterinary'
	JOIN concept_stage cs2 ON cs2.concept_code = hc.descendant_concept_code
		AND cs2.vocabulary_id = 'SNOMED Veterinary'
...
WHERE concept_name ILIKE '%score%'
	...
	AND vocabulary_id = 'SNOMED Veterinary';
...
BEGIN
    SELECT COUNT (*)
     ...
     WHERE c.vocabulary_id = 'SNOMED Veterinary' AND cs.domain_id <> c.domain_id;

    IF z <> 0
    THEN
        RAISE EXCEPTION 'Please check domain_ids for SNOMED Veterinary';
    END IF;
END $_$;*/
