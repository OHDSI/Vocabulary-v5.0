...
def load_to_db(conn, verbose=True):
    """Fetch TNM schemas and values, write to naaccr_tnm_schemas,
    naaccr_tnm_schema_inputs, and naaccr_tnm_values.  Truncates before inserting."""
    import psycopg2.extras
    s = config.DB_SOURCES_SCHEMA
    schemas, values = fetch_all(verbose=verbose)
    cur = conn.cursor()

    cur.execute(f"TRUNCATE TABLE {s}.naaccr_tnm_schemas")
    psycopg2.extras.execute_values(cur,
        f"INSERT INTO {s}.naaccr_tnm_schemas "
        f"(schema_id, schema_name, algorithm, version) VALUES %s",
        [(sc['schema_id'], sc['schema_name'], sc.get('algorithm'), sc.get('version'))
         for sc in schemas],
        page_size=500)

    cur.execute(f"TRUNCATE TABLE {s}.naaccr_tnm_schema_inputs")
    inputs = [
    ...
    psycopg2.extras.execute_values(cur,
        f"INSERT INTO {s}.naaccr_tnm_schema_inputs "
        f"(schema_id, item_number, input_name) VALUES %s",
        inputs, page_size=500)

    cur.execute(f"TRUNCATE TABLE {s}.naaccr_tnm_values")
    psycopg2.extras.execute_values(cur,
        f"INSERT INTO {s}.naaccr_tnm_values "
    ...
    if verbose:
        print(f"[tnm] Loaded {len(schemas)} rows -> {s}.naaccr_tnm_schemas, "
              f"{len(inputs)} rows -> {s}.naaccr_tnm_schema_inputs, "
              f"{len(values)} rows -> {s}.naaccr_tnm_values")
