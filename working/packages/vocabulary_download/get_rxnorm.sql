CREATE OR REPLACE FUNCTION vocabulary_download.get_rxnorm (
...
$BODY$
DECLARE
pVocabularyID constant text:='RXNORM';
...
BEGIN
  pVocabularyOperation:='GET_RXNORM';
  ...
  if iOperation not in ('ALL', 'JUMP_TO_RXNORM_PREPARE', 'JUMP_TO_RXNORM_IMPORT') then raise exception 'Wrong iOperation %',iOperation; end if;
  ...
  if pJumpToOperation='ALL' then
    ...
    --https://download.nlm.nih.gov/umls/kss/rxnorm/RxNorm_full_07022018.zip?ticket=ST-2985381-xK4ni6p5INe5DVLknMHj-cas
    ...
    --start downloading
    pVocabularyOperation:='GET_RXNORM downloading';
    perform run_wget (
    ...
    perform write_log (
      iVocabularyID=>pVocabularyID,
      iSessionID=>pSession,
      iVocabulary_operation=>'GET_RXNORM downloading complete',
      iVocabulary_status=>1
    );
  end if;
  
  if pJumpToOperation in ('ALL','JUMP_TO_RXNORM_PREPARE') then
  	pJumpToOperation:='ALL';
    --UMLS needs some preparation (MetaMorphoSys)
    pVocabularyOperation:='GET_RXNORM prepare';
    perform get_rxnorm_prepare (
      iPath=>pVocabulary_load_path,
      iFilename=>lower(pVocabularyID)||'.zip'
    );
    perform write_log (
      iVocabularyID=>pVocabularyID,
      iSessionID=>pSession,
      iVocabulary_operation=>'GET_RXNORM prepare complete',
      iVocabulary_status=>1
    );
  end if;
  
  if pJumpToOperation in ('ALL','JUMP_TO_RXNORM_IMPORT') then
  	pJumpToOperation:='ALL';
    --finally we have all input tables, we can start importing
    pVocabularyOperation:='GET_RXNORM load_input_tables';
    perform sources.load_input_tables(pVocabularyID,pVocabularyNewDate,pVocabularyNewVersion);
    perform write_log (
      iVocabularyID=>pVocabularyID,
      iSessionID=>pSession,
      iVocabulary_operation=>'GET_RXNORM load_input_tables complete',
      iVocabulary_status=>1
    );
  end if;
  ...
  perform write_log (
    iVocabularyID=>pVocabularyID,
    iSessionID=>pSession,
    iVocabulary_operation=>'GET_RXNORM all tasks done',
