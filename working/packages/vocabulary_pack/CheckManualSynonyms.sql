...
BEGIN
	...
	SELECT reason INTO z FROM (
		...
		FROM concept_synonym_manual csm
			...
			LEFT JOIN concept c_l ON c_l.concept_id = csm.language_concept_id AND c_l.vocabulary_id='SNOMED' AND c_l.domain_id='Observation' AND c_l.concept_class_id='Qualifier Value' AND c_l.concept_name ilike '%language'
	) AS s0
