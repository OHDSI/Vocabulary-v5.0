-- icd10gm_map_dif - table with better mapping for ICD10GM unique concepts
/* CREATE TABLE icd10gm_map_dif
WITH t0
AS
...
p_map AS (    
  ...
  UNION
    SELECT * FROM t2 )
SELECT * FROM p_map 
WHERE icd_code NOT IN (SELECT icd_code FROM p_map where current_name ~ '^Primary malignant|^Secondary malignant')
AND  icd_code NOT IN (SELECT icd_code FROM p_map where current_name ~ 'Finding related to pregnancy')
AND  icd_code NOT IN (SELECT icd_code FROM p_map where icd_name ~ '\s+and\s+')
ORDER BY icd_code;*/

-- concepts weren't in crm but the mapping could be better
...
-- we should deprecate previous mapping (do it in crm - they will be changed in crs after load_stage
INSERT INTO concept_relationship_manual
SELECT concept_code_1,
       concept_code_2,
       vocabulary_id_1,
