...
DO $_$ BEGIN PERFORM VOCABULARY_PACK.SetLatestUpdate (pVocabularyName => 'ICD10',pVocabularyDate =>(SELECT vocabulary_date
FROM sources.icdclaml
LIMIT 1),pVocabularyVersion =>(SELECT vocabulary_version
FROM sources.icdclaml
LIMIT 1),pVocabularyDevSchema => 'DEV_ICD10');
...
FROM (SELECT *
      FROM (SELECT (xpath ('./@code',i.xmlfield))[1]::VARCHAR modifierclass_code,
            ...
            FROM (SELECT UNNEST(xpath ('/ClaML/ModifierClass',i.xmlfield)) xmlfield
                  FROM sources.icdclaml i) AS i) AS s0) AS s1
...
FROM (SELECT *
      FROM (SELECT (xpath ('./@code',i.xmlfield))[1]::VARCHAR class_code,
            ...
            FROM (SELECT UNNEST(xpath ('/ClaML/Class',i.xmlfield)) xmlfield
                  FROM sources.icdclaml i) AS i
              LEFT JOIN LATERAL (SELECT UNNEST(xpath ('./SuperClass/@code',i.xmlfield))::VARCHAR superclass_code) l ON TRUE) AS s0) AS s1
  LEFT JOIN LATERAL (SELECT STRING_AGG(LTRIM(REGEXP_REPLACE(rubric_label,'\t','','g')),'') AS rubric_label
                     FROM (SELECT UNNEST(xpath ('//text()',s1.rubric_label))::VARCHAR rubric_label) AS s0) l ON TRUE)
--modify classes_table replacing  preferred name to preferredLong where it's possible
SELECT a.CLASS_CODE,a.RUBRIC_KIND,a.SUPERCLASS_CODE,COALESCE (b.rubric_label,a.rubric_label) AS rubric_label FROM classes a
  LEFT JOIN classes b
...
SELECT a.concept_code || b.modifierclass_code AS concept_code,
       CASE
         WHEN b.modifer_name = 'Kimmelstiel-Wilson syndromeN08.3'
...
FROM codes a
  JOIN codes b
   ...
   AND a.class_code != b.class_code)
    OR (a.rubric_label = '[See at the beginning of this block for subdivisions]'
   AND a.rubric_label = b.rubric_label
   AND a.class_code != b.class_code
   AND b.concept_code = 'K25')
    OR (a.rubric_label = '[See site code at the beginning of this chapter]'
   AND a.rubric_label = b.rubric_label
   AND a.class_code != b.class_code
...
WHERE ((a.concept_code NOT LIKE '%.%' AND b.modifierclass_code LIKE '%.%') OR (a.concept_code LIKE '%.%' AND b.modifierclass_code NOT LIKE '%.%'))
UNION
...
SELECT c1.concept_code AS concept_code_1,
       ...
       (SELECT latest_update
        FROM vocabulary
        WHERE vocabulary_id = c1.vocabulary_id) AS valid_start_date,
       TO_DATE('20991231','yyyymmdd') AS valid_end_date,
       NULL AS invalid_reason
...
ALTER TABLE concept_stage ALTER COLUMN domain_id
   SET NOT NULL;
...
WHERE 'ICD10' IN (a.vocabulary_id,b.vocabulary_id)
AND   NOT EXISTS (SELECT 1
                  FROM concept_relationship_stage crs_int
