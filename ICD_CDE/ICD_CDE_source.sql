...
--GROUPING CRITERIUM 1: identical source_code_description
DROP TABLE grouped;
...
UPDATE icd_cde_source s SET group_id =
    (SELECT m.group_id FROM cde_manual_group m WHERE m.source_code = s.source_code AND m.source_code_description = s.source_code_description AND m.source_vocabulary_id = s.source_vocabulary_id)
WHERE source_code in (SELECT source_code FROM cde_manual_group) and source_code_description in (SELECT source_code_description FROM cde_manual_group);

SELECT * FROM icd_cde_source;



--GROUPING CRITERIUM 2: (same codes with identical mappings)
DROP TABLE grouped;
CREATE TABLE grouped as (
SELECT DISTINCT
       c1.source_code as source_code,
...
INSERT INTO cde_manual_group (source_code, source_code_description, source_vocabulary_id, group_id, group_name)
...
SELECT DISTINCT source_code_1,
                ...
                source_code_description_1
    FROM grouped
WHERE (source_code_1, source_vocabulary_id_1, group_id_1) NOT IN (SELECT source_code, source_vocabulary_id, group_id FROM cde_manual_group)
;
...
CREATE TABLE icd_cde_manual as (
SELECT DISTINCT
group_name,
group_id,
...
--FOR GROUPS:
-- 1. same code, same description, same vocabulary, different mappings should be in one group (generate groups only for sources, than add mappings)
-- 2. same code, same mappings
