...
AND c.vocabulary_id = 'ICD10GM'
AND c.concept_code NOT IN (SELECT concept_code FROM concept WHERE vocabulary_id = 'ICD10')
UNION
...
AND a.concept_code NOT IN (SELECT concept_code FROM concept WHERE vocabulary_id = 'ICD10')
AND   a.invalid_reason IS NULL
...
t1 AS (SELECT d.concept_code AS icd_code,
       ...
       FROM concept_relationship_manual a
         JOIN concept b
          ...
          AND d.vocabulary_id = 'ICD10GM'
          AND d.concept_code NOT IN (SELECT concept_code FROM concept WHERE vocabulary_id = 'ICD10')
...
-- improve_map - automatically detected mapping improvements. Look carefully! Target vocabulary could have the same names of concepts with different domain_ids. Also, ICD10GM chapter means a lot and should be taken into account for choosing appropriate mapping
improve_map
AS
(SELECT DISTINCT
