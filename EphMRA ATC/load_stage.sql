...
BEGIN
	...
	pVocabularyName			=> 'EphMRA ATC',
	pVocabularyDate			=> (SELECT vocabulary_date FROM sources.atc_glossary LIMIT 1),
	pVocabularyVersion		=> (SELECT vocabulary_version FROM sources.atc_glossary LIMIT 1),
...
SELECT concept_name,
	...
	'Drug' AS domain_id,
	CASE 
		WHEN LENGTH(concept_code) = 1
			THEN 'ATC 1st'
		WHEN LENGTH(concept_code) = 2
			THEN 'ATC 2nd'
		WHEN LENGTH(concept_code) = 3
			THEN 'ATC 3rd'
		WHEN LENGTH(concept_code) = 4
			THEN 'ATC 4th'
		WHEN LENGTH(concept_code) = 5
			THEN 'ATC 5th'
...
FROM (
	SELECT UPPER(TRIM(concept_code)) AS concept_code,
		REPLACE(COALESCE(concept_name, n1, n2), '?', '-') AS concept_name
	FROM SOURCES.ATC_Glossary
	WHERE concept_code NOT LIKE '\_%' ESCAPE '\'
