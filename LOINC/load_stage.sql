...
--16 Delete ambiguous 'Maps to' mappings if target concept have domain_id='Drug' and concept_class_id NOT IN ('Ingredient', 'Clinical Drug Comp')
...
--17 Delete duplicate replacement mappings (one concept has multiply target concepts)
...
--18 Delete self-connected mappings ("A 'Concept replaced by' B" and "B 'Concept replaced by' A")
DELETE FROM concept_relationship_stage
...
--19 Deprecate concepts if we have no active replacement record in the concept_relationship_stage
...
--20 Deprecate replacement records if target concept was depreceted 
...
--21 Deprecate concepts if we have no active replacement record in the concept_relationship_stage (yes, again)
UPDATE concept_stage cs
   SET cs.valid_end_date =
          (SELECT v.latest_update - 1
             FROM VOCABULARY v
