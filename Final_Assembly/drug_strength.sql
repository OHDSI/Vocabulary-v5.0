...
select distinct
  ...
  first_value(au.target_concept_id) over (partition by drug_id, ingredient_id order by component_id rows between unbounded preceding and unbounded following) as amount_concept_id, 
...
from (
  select distinct
    ...
    numerator_unit, 
    1, -- denominator_value, in Clinical/Branded Drugs always normalized to 1
        ...
        from ( -- provide drugs with cleaned components and ingredients 
          ...
          from (
            select distinct -- select for each drug the drug_component(s) and ingredient(s), and replace the component name if necessary
              ...
              i.valid_end_date
            from concept d
            join concept_ancestor a1 on d.concept_id=a1.descendant_concept_id
            join concept c on a1.ancestor_concept_id=c.concept_id and c.concept_class_id='Clinical Drug Comp'
            join concept_ancestor a2 on c.concept_id=a2.descendant_concept_id
            join concept i on a2.ancestor_concept_id=i.concept_id and i.concept_class_id='Ingredient'
...
/* 3. Write 'Clinical Drug Components' */
      ...
      from (
        ...
        from ( -- provide drugs with cleaned components and ingredients 
          ...
          from (
            ...
            join concept i on a2.ancestor_concept_id=i.concept_id and i.concept_class_id='Ingredient'
            left join component_replace r on r.component_name=lower(c.concept_name)
            where c.standard_concept='S' and c.concept_class_id = ('Clinical Drug Comp')
          ) 
        )
      )
    )
...
/* 4. Write 'Quantified * Drugs from Clinical Drugs */

-- Quantity provided in "ACTUAT": They only exist for concentrations of ingredients
insert /*+ APPEND */ into drug_strength (
  drug_concept_id, ingredient_concept_id, amount_value, amount_unit_concept_id, numerator_value, numerator_unit_concept_id, denominator_value, denominator_unit_concept_id, valid_start_date, valid_end_date, invalid_reason
)
select distinct
  drug_concept_id, -- of the quantified
  ingredient_concept_id, -- of the original non-quantified
...
from (
  select 
    q.concept_id as drug_concept_id, ds.ingredient_concept_id, 
  ...
  join concept d on d.concept_id=ds.drug_concept_id and d.concept_class_id in ('Clinical Drug', 'Branded Drug')
  join concept_relationship r on r.concept_id_1=ds.drug_concept_id and r.invalid_reason is null
  join concept q on q.concept_id=r.concept_id_2 and q.concept_class_id like 'Quant%'
)
where u='ACTUAT';
