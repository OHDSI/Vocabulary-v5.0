CREATE OR REPLACE FUNCTION dev_veterinary.GenericUpdate_VET (
)
RETURNS void AS
$BODY$
BEGIN
	...
	-- 4.5. SNOMED Veterinary extension: transfer vocabulary ownership for concepts
	-- that exist in concept_stage as 'SNOMED Veterinary' but whose live concept
	-- table entry is under a different vocabulary (typically 'Nebraska Lexicon').
	-- This occurs when the vet extension's module (332351000009108) claims
	-- ownership of a concept code that OHDSI's Nebraska Lexicon vocabulary
	-- currently holds. GenericUpdate()'s standard step 4 cannot match these
	-- rows (concept_code matches but vocabulary_id differs), so without this
	-- step they would be silently skipped and remain as Nebraska Lexicon
	-- concepts indefinitely, despite the vet extension's explicit module claim.
	--
	-- This step:
	--   a) Updates the live concept row's vocabulary_id to 'SNOMED Veterinary'
	--   b) Sets concept_id in concept_stage so step 11's INSERT is skipped
	--      (preventing a duplicate concept for the same code)
	--
	-- Only applies to concept_stage rows that still have concept_id IS NULL
	-- after step 4 (i.e. no existing 'SNOMED Veterinary' row was found),
	-- where an active live concept exists for the same concept_code under
	-- a different vocabulary.
	--
	-- Vocabularies affected (confirmed by diagnostic query, 2026-06):
	--   Nebraska Lexicon: 3029 concepts (primarily Organism breed/species)
	--   OSM: 2 concepts
	--   MedDRA, Multilex, NCCD, RxNorm, DA_France, VANDF, LPD_Australia: 1 each
	--
	-- NOTE: This modification is specific to the SNOMED Veterinary build and
	-- should not be applied to devv5.GenericUpdate() without broader review,
	-- as it may have unintended consequences for other vocabularies that also
	-- use Nebraska Lexicon or similar cross-vocabulary ownership patterns.
	-- Build a temp table of exactly one candidate row per concept_code to
	-- transfer, to avoid updating multiple rows for the same code and
	-- violating the unique constraint on (vocabulary_id, concept_code).
	-- Priority order prefers Nebraska Lexicon (the most common case) over
	-- other displaced-SNOMED vocabularies.
	CREATE TEMP TABLE vet_transfer_candidates$ AS
	...
	WHERE cs.concept_id IS NULL
		...
		AND NOT EXISTS (
			SELECT 1 FROM concept c2
			WHERE c2.concept_code = cs.concept_code
				AND c2.vocabulary_id = 'SNOMED Veterinary'
				AND c2.invalid_reason IS NULL
		)
	ORDER BY c.concept_code,
		CASE c.vocabulary_id
			WHEN 'Nebraska Lexicon' THEN 1
			WHEN 'OSM'             THEN 2
			ELSE                        3
		END;

	UPDATE concept c
	SET vocabulary_id    = 'SNOMED Veterinary',
	...
	FROM vet_transfer_candidates$ t
	WHERE c.concept_id = t.concept_id;

	DROP TABLE vet_transfer_candidates$;

	-- Now set concept_id in concept_stage for the transferred rows so step 11
	-- treats them as existing concepts and does not INSERT duplicates.
	UPDATE concept_stage cs
		SET concept_id = c.concept_id
	FROM concept c
	WHERE cs.concept_code = c.concept_code
	...
	****************************/
	...
	WHERE c.vocabulary_id = v.vocabulary_id
		AND COALESCE(v.vocabulary_params ->> 'is_full', '0') = '1'
		...
		SELECT concept_id + 1 INTO ex FROM (
			...
			FROM (
				...
				(
					...
					UNION ALL
					SELECT concept_id FROM devv5.concept_blacklisted
				) AS i
	...
	****************************************/
	...
	--16. Deprecate missing relationships
	...
	--17. Deprecate old 'Maps to' and replacement records
	WITH relationships AS (
			...
			CASE 
                WHEN GREATEST(r.valid_start_date, 
                              (SELECT MAX(v.latest_update) -1
                                 FROM vocabulary v
                                WHERE v.vocabulary_id IN (c1.vocabulary_id, c2.vocabulary_id))
                     ) < r.valid_start_date
                THEN CURRENT_DATE - 1
	...
	UPDATE concept_relationship r
	SET valid_end_date  =
			GREATEST(r.valid_start_date, (SELECT MAX(v.latest_update) -1
				FROM vocabulary v
			WHERE v.vocabulary_id IN (c1.vocabulary_id, c2.vocabulary_id)
			)),
		invalid_reason = 'D'
	...
	--19. Make sure invalid_reason = 'U' if we have an active replacement record
	UPDATE concept c
	...
	--20. Make sure invalid_reason = 'D' if we have no active replacement record
	UPDATE concept c
	SET valid_end_date = LEAST(c.valid_end_date, v.latest_update - 1),
		invalid_reason = 'D',
		standard_concept = NULL
	...
	--21. 'Maps to' and 'Mapped from' relationships from concepts to self
	...
	--22. 'Maps to' or 'Maps to value' relationships cleanup
	UPDATE concept_relationship r
	SET valid_end_date = GREATEST(r.valid_start_date, (SELECT MAX(v.latest_update)-1 FROM vocabulary v WHERE v.vocabulary_id=c1.vocabulary_id OR v.vocabulary_id=c2.vocabulary_id)),
		invalid_reason = 'D'
	...
	AND r.concept_id_2 = c2.concept_id
	AND (
		(c2.standard_concept = 'S' AND c1.concept_id <> c2.concept_id)
		OR COALESCE (c1.standard_concept, 'X') <> 'S'
		OR c1.invalid_reason IN ('U', 'D')
	)
	AND v.vocabulary_id IN (c1.vocabulary_id, c2.vocabulary_id)
	AND v.latest_update IS NOT NULL
	...
	************************************/
	...
	--24. Remove duplicates from concept_synonym_stage
	DELETE
	...
	--25. Remove synonyms if synonym_name already exists in concept_stage
	DELETE
	FROM concept_synonym_stage css
	WHERE EXISTS (
			SELECT 1
