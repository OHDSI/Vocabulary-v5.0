...
IS
   ...
   END;
    ...
    FUNCTION GetHTMLProper (pURL IN VARCHAR2)
        RETURN CLOB
    IS
        /*
        The site 'isd.hscic.gov.uk' doens't have the DNS reverse zone (HTTPURITYPE will fail with 'ORA-24263: Certificate of the remote server does not match the target address.'),
        so we need to use additional parameter 'https_host' in UTL_HTTP package
        BTW: https://www.accessdata.fda.gov and https://loinc.org/downloads <- same issue
        */
        cPieces    UTL_HTTP.HTML_PIECES;
    ...
    BEGIN
        ...
        ELSIF LOWER (SUBSTR (pURL, 1, 17)) = 'https://loinc.org'
        THEN
            cPieces := UTL_HTTP.REQUEST_PIECES (url => pURL, https_host => '*.loinc.org');
        ELSE
            cPieces := UTL_HTTP.REQUEST_PIECES (url => pURL);
        END IF;
        ...
        THEN
            html_out := NULL;
        ELSE
            FOR i IN 1 .. cPieces.COUNT
            LOOP
                html_out := html_out || TO_CLOB (cPieces (i));
            END LOOP;
        END IF;
    ...
    BEGIN
        ...
        */
        ...
        --UTL_HTTP.set_wallet ('file:/home/oracle/wallet', 'wallet_password');
        --cVocabHTML := HTTPURITYPE (cURL).getCLOB (); --can't using HTTPURITYPE, see comments in GetHTMLProper
        cVocabHTML := GetHTMLProper (cURL);
        ...
        CASE
            ...
            THEN
                ...
                cPos1 := INSTR (cVocabHTML, cSearchString, cPos1);
                cPos2 := INSTR (cVocabHTML, ' ', cPos1 + LENGTH (cSearchString) + 1);
                CheckPositions (cPos1, cPos2);
            ...
            THEN
                     SELECT TO_DATE (REGEXP_SUBSTR (TRIM (title), '[[:alpha:]]+ [[:digit:]]+$'), 'mon yyyy')
                       ...
                       FROM XMLTABLE ('/rss/channel/item'
                                      PASSING xmltype (cVocabHTML)
                                      COLUMNS link_str VARCHAR2 (500) PATH 'link', pubDate VARCHAR2 (500) PATH 'pubDate', title VARCHAR2 (500) PATH 'title') t
