CREATE OR REPLACE FUNCTION vocabulary_pack.pConceptAncestor (is_small BOOLEAN = FALSE)
RETURNS VOID AS
$BODY$
	/*
	The function allows to build a full concept ancestor or small, with a specific set of vocabularies ('RxNorm, RxNorm Extension, ATC, NFC, EphMRA ATC, CVX')

	Examples:
	--full concept ancestor:
	DO $_$
	BEGIN
		PERFORM vocabulary_pack.pConceptAncestor();
	END $_$;

	--small concept ancestor
	DO $_$
	BEGIN
...
BEGIN
	...
	IF is_small THEN 
		iVocabularies:='RxNorm, RxNorm Extension, ATC, NFC, EphMRA ATC, CVX';
	END IF;

	PERFORM vocabulary_pack.ConceptAncestorCore(iVocabularies, FALSE);
	...
	CREATE INDEX idx_temp_ca_rx$ ON concept_ancestor_rx$ (ancestor_concept_id,descendant_concept_id) WITH (FILLFACTOR=100);
	...
	CREATE INDEX idx_jump_table ON jump_table (rxn_concept_id,class_concept_id) WITH (FILLFACTOR=100);
	ANALYZE jump_table;
	...
	CREATE INDEX idx_excluded_concepts ON excluded_concepts (descendant_concept_id,class_concept_id) WITH (FILLFACTOR=100);
	...
	CREATE INDEX idx_pair ON pair_tbl (class_concept_id) WITH (FILLFACTOR=100);
	ANALYZE pair_tbl;
...
--create table with wrong relationships (non-neighbor relationships)
							...
							WHERE c.vocabulary_id IN (
								...
								AND NOT EXISTS (
									...
									WHERE ds.drug_concept_id = c.concept_id
										AND NOT (
											ds.amount_unit_concept_id IS NOT NULL
											AND ds.amount_value IS NULL
											)
									)
							) AS s1
	...
	upsert_to_ds
	AS (
		INSERT INTO drug_strength AS ds
		SELECT *
		FROM insert_to_ds ON CONFLICT ON CONSTRAINT xpk_drug_strength
		DO UPDATE
		SET amount_value = NULL,
			amount_unit_concept_id = excluded.amount_unit_concept_id,
		...
		WHERE ds.* IS DISTINCT FROM excluded.*
		)
	DELETE
	FROM drug_strength ds
	WHERE ds.amount_unit_concept_id IS NOT NULL
		...
		AND NOT EXISTS (
			SELECT 1
			FROM insert_to_ds ds_int
			WHERE ds_int.drug_concept_id = ds.drug_concept_id
				AND ds_int.ingredient_concept_id = ds.ingredient_concept_id
			);
