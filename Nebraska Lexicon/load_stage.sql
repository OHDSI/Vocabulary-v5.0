...
BEGIN
	...
	pVocabularyName			=> 'Nebraska Lexicon',
	pVocabularyDate			=> (SELECT vocabulary_date FROM sources.lex_sct2_concept LIMIT 1),
	pVocabularyVersion		=> (SELECT vocabulary_version FROM sources.lex_sct2_concept LIMIT 1),
	pVocabularyDevSchema	=> 'DEV_LEXICON'
);
END $_$;
...
--3. Create core version of Nebraska Lexicon
...
FROM (
	...
	FROM sources.lex_sct2_concept c,
		sources.lex_sct2_desc d
...
INSERT INTO concept_relationship_stage (
	...
	FROM (
		SELECT sc.referencedcomponentid AS concept_code_1,
			...
			active
		FROM sources.lex_der2_crefset_assref sc
...
WHERE cs.invalid_reason IS NULL
	...
	AND cs.vocabulary_id = 'Nebraska Lexicon'
...
--18. Delete mappings between concepts that are not represented at the "latest_update" at this moment (e.g. SNOMED <-> RxNorm, but currently we are updating Nebraska Lexicon)
--This is because we have SNOMED <-> Nebraska Lexicon in concept_relationship_stage, but AddFreshMAPSTO adds SNOMED <-> RxNorm from concept_relationship
DELETE
