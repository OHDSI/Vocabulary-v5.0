...
FROM (
with tab as (SELECT b.concept_id, vocabulary_pack.CutConceptName(regexp_split_to_table(gdna,'__'))  as concept_name,trim(substr(regexp_split_to_table(gdna,'__'),1,50)) as concept_code,valid_start_date
FROM dev_cgi.genomic_cgi_source a
JOIN concept b
 ON   concat(a.gene, ':', regexp_replace(a.protein, 'p.', '')) = b.concept_code
and b.vocabulary_id='CGI')
    SELECT concept_id, concept_name, concept_code,valid_start_date
from tab
