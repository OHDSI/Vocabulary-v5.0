...
merge into supplier_2 s
using (select concept_code,concept_name from devv5.concept where concept_class_id='Supplier' and vocabulary_id='AMT') c on (s.supplier=c.concept_name)
when matched then update
set s.concept_code=c.concept_code;
...
update dcs_bn 
set concept_name='Feiba-NF' where regexp_like(concept_name,'Feiba-NF');
update dcs_bn 
set concept_name='Xylocaine' where regexp_like(concept_name,'Xylocaine');
update dcs_bn 
set concept_name='Canesten' where regexp_like(concept_name,'Canesten');
update dcs_bn 
...
update  dcs_bn
set concept_name=replace(concept_name,' IM$| IV$');
...
;
update dcs_bn
set concept_name=trim(regexp_replace(concept_name,'  ',' '));
...
delete dcs_bn where concept_name like '% mg%' or concept_name in ('Aciclovir Intravenous','Aciclovir IV','Acidophilus Bifidus','Risperidone','Ropivacaine','Piperacillin And Tazobactam','Perindopril And Indapamide','Paracetamol IV',
...
delete dcs_bn where ( concept_name like '%artan % HCT%' or concept_name like '%Sodium% HCT%') and concept_name!='Asartan HCT';--delete combination of ingredients
...
select distinct CONCEPT_NAME, 'AMT', NEW_CONCEPT_CLASS_ID, '', CONCEPT_CODE, '','Drug', TO_DATE('2016/11/01', 'yyyy/mm/dd') as valid_start_date,TO_DATE('2099/12/31', 'yyyy/mm/dd') as valid_end_date, '',CONCEPT_CLASS_ID
 from 
(
select distinct CONCEPT_NAME,'Ingredient' as NEW_CONCEPT_CLASS_ID,CONCEPT_CODE,CONCEPT_CLASS_ID from concept_stage_sn  where CONCEPT_CLASS_ID='AU Substance' and concept_code not in ('52990011000036102','48158011000036109')-- Aqueous Cream ,Cotton Wool
union 
...
insert into DRUG_concept_STAGE (CONCEPT_NAME,VOCABULARY_ID,CONCEPT_CLASS_ID,STANDARD_CONCEPT,CONCEPT_CODE,POSSIBLE_EXCIPIENT,domain_id,VALID_START_DATE,VALID_END_DATE,INVALID_REASON, SOURCE_CONCEPT_CLASS_ID)
select distinct CONCEPT_NAME, 'AMT', 'Device', 'S', CONCEPT_CODE, '','Device', TO_DATE('2016/11/01', 'yyyy/mm/dd') as valid_start_date,TO_DATE('2099/12/31', 'yyyy/mm/dd') as valid_end_date, '',CONCEPT_CLASS_ID
