CREATE OR REPLACE FUNCTION apigrabber.GetRxNorm2SPL_Mappings (
)
RETURNS void AS
$body$
declare
  ...
  cJOBName constant varchar (1000):= 'JOB "RxNorm2SPL_Mappings"';
...
begin
    ...
    if z=0 or cCode is null then  --all ok OR all errors are gone
        select count(*) into cPrevMappingsCount from apigrabber.rxnorm2spl_mappings;
        select count(*) into cFreshMappingsCount from apigrabber.rxnorm2spl_mappings_tmp;
        truncate apigrabber.rxnorm2spl_mappings;
        insert into apigrabber.rxnorm2spl_mappings select * from apigrabber.rxnorm2spl_mappings_tmp;
        truncate apigrabber.rxnorm2spl_mappings_tmp;
        analyze apigrabber.rxnorm2spl_mappings;
