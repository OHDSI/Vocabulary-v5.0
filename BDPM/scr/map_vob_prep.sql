...
*/
left join (
select drug_concept_code, denominator_value as quant_f from ds_stage where denominator_value is not null) q_qnt on q_qnt.drug_concept_code=m.q_dcode
left join 
(
select drug_concept_id, denominator_value as quant_f  from drug_strength where denominator_value is not null) r_qnt on r_qnt.drug_concept_id=m.r_did
