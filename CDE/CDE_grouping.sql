...
select * from cde_merge_group('cde_manual_group', 2, ARRAY['H40:ICD10']);
...
select * from cde_merge_group(ARRAY[1,2]); -- Case 2a works -- works in terms of group_id, but not in terms of group_name and group_code
SELECT * FROM cde_manual_group;
 ...
 * 		select * from cde_split_group('cde_manual_group',2);
 *
 *  --Detach selected group code values into a new group
 *		select * from cde_split_group('cde_manual_group',2, ARRAY['Z96.7:ICD10']);
 */
DECLARE
	V_SEQUENCE_VAL INT;
	z int4;
...
SELECT * FROM cde_manual_group;
SELECT * FROM cde_split_group('cde_manual_group', 2);
select * from cde_split_group('cde_manual_group',2, ARRAY['Z99.201:ICD10CN']);
...
select * from cde_merge_group('cde_manual_group',ARRAY[1,2]); -- works in terms of group_id, but not in terms of group_name and group_code
SELECT * FROM cde_manual_group;
