...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.concept c ON c.concept_id = t.concept_id::integer
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.concept c_code ON c_code.concept_code = t.concept_code
  AND c_code.vocabulary_id = t.vocabulary_id
LEFT JOIN vocab.concept c_id ON c_id.concept_id = t.concept_id::integer
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.vocabulary v ON v.vocabulary_id = t.vocabulary_id_1
WHERE v.vocabulary_id IS NULL
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.vocabulary v ON v.vocabulary_id = t.vocabulary_id_2
WHERE v.vocabulary_id IS NULL
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.concept c_code ON c_code.concept_code = t.concept_code_1
  AND c_code.vocabulary_id = t.vocabulary_id_1
LEFT JOIN vocab.concept c_id ON c_id.concept_id = t.concept_id_1::integer
WHERE c_code.concept_id IS NULL
  AND c_id.concept_id IS NULL;
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.concept c_code ON c_code.concept_code = t.concept_code_2
  AND c_code.vocabulary_id = t.vocabulary_id_2
LEFT JOIN vocab.concept c_id ON c_id.concept_id = t.concept_id_2::integer
WHERE c_code.concept_id IS NULL
  AND c_id.concept_id IS NULL;
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.vocabulary v ON v.vocabulary_id = t.synonym_vocabulary_id
WHERE v.vocabulary_id IS NULL
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.concept c ON c.concept_code = t.synonym_concept_code
...
FROM {TEMP_TABLE} t
LEFT JOIN vocab.concept c ON c.concept_id = t.language_concept_id::integer
WHERE c.concept_id IS NULL
...
SELECT
  ...
  'language_concept_id' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept c ON c.concept_id = t.language_concept_id::integer
...
SELECT
  ...
  'synonym_name' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept c ON c.concept_code = t.synonym_concept_code
  AND c.vocabulary_id = t.synonym_vocabulary_id
INNER JOIN vocab.concept_synonym cs ON cs.concept_id = c.concept_id
...
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept c ON c.concept_id = t.concept_id_2::integer
WHERE c.standard_concept IS DISTINCT FROM 'S'
...
SELECT
  ...
  'concept_code_2' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept c ON c.concept_code = t.concept_code_2
...
SELECT
  ...
  'relationship_id' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept c1 ON c1.concept_code = t.concept_code_1
  AND c1.vocabulary_id = t.vocabulary_id_1
INNER JOIN vocab.concept c2 ON c2.concept_code = t.concept_code_2
...
SELECT
  ...
  'vocabulary_id_1' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.vocabulary v ON v.vocabulary_id = t.vocabulary_id_1;
...
SELECT
  ...
  'concept_code_1' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept c ON c.concept_code = t.concept_code_1
...
SELECT
  ...
  'concept_id' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept c ON c.concept_id = t.concept_id::integer
...
INNER JOIN vocab.concept c1 ON c1.concept_code = t.concept_code_1
  AND c1.vocabulary_id = t.vocabulary_id_1
LEFT JOIN vocab.concept c2 ON c2.concept_code = t.concept_code_2
  AND c2.vocabulary_id = t.vocabulary_id_2
LEFT JOIN vocab.concept_relationship cr
  ON cr.concept_id_1 = c1.concept_id
  AND cr.concept_id_2 = c2.concept_id
...
SELECT
  ...
  'concept_name' AS field_name
FROM {TEMP_TABLE} t
INNER JOIN vocab.concept src ON src.concept_id = t.concept_id::integer
INNER JOIN vocab.concept c ON LOWER(c.concept_name) = LOWER(src.concept_name)
