# PostgreSQL variant of collect_checks.R
# Identical logic and output — only the connection and two Spark-incompatible
# SQL files differ.  All OHDSI-SQL files from ../sql/ are reused as-is;
# DatabaseConnector auto-translates them to PostgreSQL syntax at runtime.

library(openxlsx)
library(DatabaseConnector)
library(readr)

source("R/config.R")   # loads schema variables, sqlDir, outputDir, etc.

# ── PostgreSQL connection ─────────────────────────────────────────────────────
# First-time credential setup (run once in the R console):
#   keyring::key_set("postgres_vocab", "host")       # e.g. myserver.postgres.database.azure.com
...
connectionDetailsPg <- DatabaseConnector::createConnectionDetails(
  dbms     = "postgresql",
  server   = paste0(keyring::key_get("postgres_vocab", "host"), "/",
                    keyring::key_get("postgres_vocab", "database")),
  port     = as.integer(keyring::key_get("postgres_vocab", "port")),
  user     = keyring::key_get("postgres_vocab", "user"),
  password = keyring::key_get("postgres_vocab", "password")
)

# SQL folder for PostgreSQL-specific overrides (Spark-incompatible aggregations)
pgSqlDir <- file.path("postgresql", "sql")

conn <- DatabaseConnector::connect(connectionDetailsPg)
...
DrMapChangSQL <- read_file(file.path(pgSqlDir, "drug_map_dif.sql"))
...
lost_leg_sql <- read_file(file.path(pgSqlDir, "lost_leg_of_mapping.sql"))
