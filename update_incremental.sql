/************************************************************************************************
* This is a script to update the tables concept, concept_relationship and concept_synonym
* in the Standardized Vocabulary. It expects the tables concept_stage, concept_relationship_stage
* and concept_synonym_stage to be filled with a current snapshot of active records
* and performs an incremental update.
*************************************************************************************************/

--1 Fill concept_id where record (combination of concept_code and vocabulary_id) exists in the CONCEPT table
              ...
              (SELECT 1
                 FROM (SELECT rs.concept_id_1,
                       ...
                       SELECT r.concept_id_1,
                              r.concept_id_2,
                              r.relationship_id
                         FROM concept_relationship r) d
                WHERE     c.concept_id IN (d.concept_id_1, d.concept_id_2)
                      AND d.relationship_id IN ('UCUM replaced by',
                                                'UCUM replaces',
                                                'Concept replaced by',
                                                'Concept replaces',
                                                'Concept same_as to',
