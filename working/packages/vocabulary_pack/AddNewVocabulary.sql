...
DECLARE
  ...
  ex INT;
begin
  IF COALESCE(pOMOP_req,'Y') <> 'Y' THEN RAISE EXCEPTION 'pOMOP_req must be NULL or Y'; END IF;
  ...
  WHERE (concept_id>=200 and concept_id<1000 and pSEQ_VIP_gen=TRUE) --only for VIP concepts
  OR (concept_id >= 31967 AND concept_id < 72245 and pSEQ_VIP_gen=FALSE);
