...
select  distinct concept_code_1, r.concept_id, '1'    -- Map Marketed Form to corresponding Branded/Clinical Drug (save box size and quant factor)
from r_existing r
  join ex e
    on r.quant_value = e.r_value and r.quant_unit_id = e.quant_unit_id
   and r.i_combo = e.ri_combo and r.d_combo = e.rd_combo and r.df_id = e.df_id and r.bn_id = e.bn_id and r.bs = e.bs
   and r.mf_id=0
  ...
  where concept_code_1 not in (select from_code from map_drug);
 
insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '2'    -- Kick box size out
from r_existing r
  join ex e
    on r.quant_value = e.r_value and r.quant_unit_id = e.quant_unit_id
   and r.i_combo = e.ri_combo and r.d_combo = e.rd_combo and r.df_id = e.df_id and r.bn_id = e.bn_id
...
insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '3'  -- Kick Quant factor out
from r_existing r
  join ex e
    on r.i_combo = e.ri_combo and r.d_combo = e.rd_combo and r.df_id = e.df_id and r.bn_id = e.bn_id
...
insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '3'  -- Kick BN out, save Quant factor 
from r_existing r
  join ex e
    on r.i_combo = e.ri_combo and r.d_combo = e.rd_combo and r.df_id = e.df_id 
...
insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '4'  -- Map Branded Drug to corresponding Clinical Drug (save box size)
from r_existing r
  join ex e
    on r.i_combo = e.ri_combo and r.d_combo = e.rd_combo and r.df_id = e.df_id and r.bs = e.bs
 ...
 insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '5'  -- Map Branded Drug to corresponding Clinical Drug
from r_existing r
  join ex e
    on r.i_combo = e.ri_combo and r.d_combo = e.rd_combo and r.df_id = e.df_id
...
insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '6'  -- Branded Drug Form
from r_existing r
  join ex e
    on r.i_combo = e.ri_combo and r.df_id = e.df_id and r.bn_id = e.bn_id 
...
insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '7'  -- Branded Drug Com
from r_existing r
  join ex e
 ...
 insert into map_drug (from_code, to_id, map_order)
select  distinct concept_code_1, r.concept_id, '8'  -- Clinical Drug Form
from r_existing r
  join ex e
    on r.i_combo = e.ri_combo and r.df_id = e.df_id 
...
select distinct concept_code_1, r_concept_id, '9' -- Clinical Drug Comp
