...
FROM devv5.concept c
         JOIN devv5.concept_relationship cr ON cr.concept_id_1 = c.concept_id AND c.vocabulary_id IN ('RxNorm', 'RxNorm Extension')
...
FROM devv5.concept c
         JOIN devv5.concept_ancestor ca ON c.concept_id = ca.descendant_concept_id AND c.vocabulary_id IN ('RxNorm', 'RxNorm Extension')
...
FROM dev_atc.concept_stage cs
         JOIN dev_atc.concept_relationship_stage crs ON crs.concept_code_1 = cs.concept_code AND cs.concept_class_id = 'ATC 5th'
                                                                                             AND crs.invalid_reason IS NULL
                                                                                             AND crs.relationship_id = 'ATC - RxNorm'
         JOIN devv5.concept c ON c.concept_id = crs.concept_id_2
WHERE cs.concept_code IN (SELECT class_code
                          FROM dev_atc.class_to_drug_manual)
...
FROM dev_atc.concept_stage cs
         JOIN dev_atc.concept_relationship_stage crs ON crs.concept_code_1 = cs.concept_code AND cs.concept_class_id = 'ATC 5th'
                                                                                             AND crs.invalid_reason IS NULL
                                                                                             AND crs.relationship_id = 'ATC - RxNorm'
         JOIN rx ON rx.concept_id = crs.concept_id_2 AND n_ings = 1
WHERE NOT EXISTS (SELECT *
                  FROM dev_atc.concept_relationship_stage crs2
                  WHERE crs2.concept_code_1 = crs.concept_code_1
                    AND (
                        crs2.relationship_id LIKE '%sec%' --or crs2.relationship_id like '%pr up%')
                        AND
                        crs2.invalid_reason IS NULL
                        )
                  )
  AND (cs.concept_code, rx.concept_id) NOT IN (SELECT class_code, concept_id
                                               FROM class_to_drug);
...
FROM dev_atc.concept_stage cs
         JOIN dev_atc.concept_relationship_stage crs ON crs.concept_code_1 = cs.concept_code AND cs.concept_class_id = 'ATC 5th'
                                                                                             AND crs.invalid_reason IS NULL
                                                                                             AND crs.relationship_id = 'ATC - RxNorm'
         JOIN rx ON rx.concept_id = crs.concept_id_2 AND n_ings = 2
WHERE NOT EXISTS (SELECT *
                  FROM dev_atc.concept_relationship_stage crs2
                  WHERE crs2.concept_code_1 = crs.concept_code_1
...
FROM dev_atc.concept_stage cs
         JOIN dev_atc.concept_relationship_stage crs ON crs.concept_code_1 = cs.concept_code AND cs.concept_class_id = 'ATC 5th'
                                                                                             AND crs.invalid_reason IS NULL
                                                                                             AND crs.relationship_id = 'ATC - RxNorm'
         JOIN rx ON rx.concept_id = crs.concept_id_2 AND n_ings IN (3, 4)
WHERE NOT EXISTS (SELECT *
                  FROM dev_atc.concept_relationship_stage crs2
                  WHERE crs2.concept_code_1 = crs.concept_code_1
                    AND (crs2.relationship_id LIKE '%up%')
...
FROM dev_atc.concept_stage cs
         JOIN dev_atc.concept_relationship_stage crs ON crs.concept_code_1 = cs.concept_code AND cs.concept_class_id = 'ATC 5th'
                                                                                             ...
                                                                                             AND crs.relationship_id = 'ATC - RxNorm'
         JOIN rx ON rx.concept_id = crs.concept_id_2
WHERE (cs.concept_code, rx.concept_id) NOT IN (SELECT class_code, concept_id
                                               FROM class_to_drug);