...
JOIN sources.dpd_active_ingredients_all ai
ON ai.drug_code = s.drug_code
WHERE upper(trim(brand_name)) ~ ('NUTRILITE|ABC PLASTER|PROVOCHOLINE|2IN1 MEN ADVANCED|ARRID|SPRAY ON|FLOSS|JOHNSON|MAKE.?UP|WRINKLE|NIVEA|ANTISEPTIQ|BLEMISH|DEODORANT|\.?SPF\.?|(\sFPS)|\sLIP\s|MOISTUR(E|IZ)|SUNSCREEN|\s?SUN\s?BLOCK|ANTI(-?)PERS|\sCLEAR\s|' ||
