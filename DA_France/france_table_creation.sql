...
;
/*
1. сделать металлы как комбинацию - done
2. положить в dose forms Американские имена - жду ответа Кристиана
3. переделать ручной update, который я использовал для лекарственных таблиц done
4. доделать комментарии потом*/ 
;
--due to issues in source table we have several Brand names for the same Brand drug, so we need to remain only one
-- creating list of duplicates having longer length of concept name
