...
BEGIN
	...
	CREATE TEMP TABLE new_relationships_prepare$ AS
		WITH upgraded_concepts_prepare AS (
				...
				LEFT JOIN concept_relationship_stage crs ON crs.concept_code_1 = c1.concept_code
					AND crs.vocabulary_id_1 = c1.vocabulary_id
					AND crs.relationship_id = r.relationship_id
					AND crs.invalid_reason IS NULL
				WHERE r.concept_id_1 <> r.concept_id_2
					AND crs.concept_code_1 IS NULL --we need only fresh 'Maps to' which contains in stage-tables (per each concept_code_1), but if we doesn't have them - take from base tables
		...
		upgraded_concepts AS (
			...
			SELECT ucp.concept_code_1,
				ucp.vocabulary_id_1,
				ucp.concept_code_2,
				ucp.vocabulary_id_2
			FROM upgraded_concepts_prepare ucp
			WHERE ucp.rel_id = 6 --Maps to
			)
		SELECT * FROM upgraded_concepts;

		CREATE INDEX idx_new_relationships_prepare$ ON new_relationships_prepare$ (concept_code_1,vocabulary_id_1) WITH (FILLFACTOR=100);
		ANALYZE new_relationships_prepare$;
		...
		WITH RECURSIVE rec AS (
				...
				FROM new_relationships_prepare$ u
				...
				FROM new_relationships_prepare$ uc
	...
	DROP TABLE new_relationships_prepare$, new_relationships$;
END;
$BODY$
LANGUAGE 'plpgsql';