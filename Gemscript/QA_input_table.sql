...
--nobody cares here in Gemscript
...
--not sure about this, what happens to those non-standard ingedients in CNDV
...
;
--5min 40 sec to proceed 
;
 select distinct numerator_unit, 'unit doesnt exist in concept_table' from ds_stage where
 ( amount_unit not in (select concept_code from drug_concept_stage where concept_class_id ='Unit')
or numerator_unit not in (select concept_code from drug_concept_stage where concept_class_id ='Unit')
or denominator_unit not in (select concept_code from drug_concept_stage where concept_class_id ='Unit')
)
union
 select distinct amount_unit, 'unit doesnt exist in concept_table' from ds_stage where
 ( amount_unit not in (select concept_code from drug_concept_stage where concept_class_id ='Unit')
or numerator_unit not in (select concept_code from drug_concept_stage where concept_class_id ='Unit')
or denominator_unit not in (select concept_code from drug_concept_stage where concept_class_id ='Unit')
)
...
;
select * from concept where concept_name like '%synalar N%'
;
select * from ds_stage ds
join drug_concept_stage dc on dc.concept_code = ds.drug_concept_code
where numerator_unit='mg' and denominator_unit='mg' and (numerator_value/denominator_value>1 
or (numerator_value>1 and denominator_vALUE IS NULL)) 
;
--wrong dosages ,> 1
select * from ds_stage ds
...
and numerator_value / denominator_value > 1
;
select * from DS_all_tmp where concept_code= '89838998'
;
select * from DS_all where concept_code= '89838998'
;
select * from ds_all 
...
;
select * from ds_all  where regexp_like (concept_name, '[[:digit:]\.]+(litre|ml)') 
and not regexp_like (concept_name, '/[[:digit:]\.]+(litre|ml)') and amount_value is not null and AMOUNT_UNIT in ('litre', 'ml')
;
-- need to comment
update ds_all set denominator_value = regexp_substr (regexp_substr(concept_name, ' [[:digit:]\.]+(litre(s?)|ml)') , '[[:digit:]\.]+'),
 denominator_unit = regexp_substr (regexp_substr(concept_name, ' [[:digit:]\.]+(litre(s?)|ml)') , '(litre(s?)|ml)')
;
select  regexp_substr (regexp_substr(concept_name, ' [[:digit:]\.]+(litre(s?)|ml)') , '[[:digit:]\.]+') as new_denom_value,
 regexp_substr (regexp_substr(concept_name, ' [[:digit:]\.]+(litre(s?)|ml)') , '(litre(s?)|ml)') as new_denom_unit, a.* from ds_all a
...
;
select * from thin_need_to_map where regexp_Like (thin_name,' [[:digit:]\.]+(litre(s?)|ml)') and domain_id = 'Drug'
;
