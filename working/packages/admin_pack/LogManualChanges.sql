...
BEGIN
	...
	EXECUTE FORMAT ($$
		SELECT * FROM (
			--manual relationships
			SELECT 1 FROM %1$I.concept_relationship_manual local_crm
			...
			--manual concepts
			SELECT 1 FROM %1$I.concept_manual local_cm
			...
			--manual synonyms
			SELECT 1 FROM %1$I.concept_synonym_manual local_csm
	...
	IF SESSION_USER<>'devv5' THEN
		...
		EXECUTE FORMAT ($$
			...
			SET valid_start_date = excluded.valid_start_date,
				...
				modified_by = %2$L
			WHERE ROW (logged_crm.valid_start_date, logged_crm.valid_end_date, logged_crm.invalid_reason)
			IS DISTINCT FROM
		...
		EXECUTE FORMAT ($$
			...
			SET concept_name = excluded.concept_name,
				...
				modified = CLOCK_TIMESTAMP(),
				modified_by = %2$L
			WHERE ROW (logged_cm.concept_name, logged_cm.domain_id, logged_cm.concept_class_id, logged_cm.standard_concept, logged_cm.valid_start_date, logged_cm.valid_end_date, logged_cm.invalid_reason)
			IS DISTINCT FROM
	...
	ELSE
		...
		PERFORM FROM devv5.concept_relationship_manual local_crm
		LEFT JOIN devv5.base_concept_relationship_manual base_crm USING (
				concept_code_1,
				concept_code_2,
				vocabulary_id_1,
		...
		WHERE ROW (local_crm.valid_start_date, local_crm.valid_end_date, local_crm.invalid_reason)
			...
			AND NOT EXISTS (
				SELECT 1
				FROM concept_relationship_manual logged_crm
				WHERE logged_crm.concept_code_1 = local_crm.concept_code_1
		...
		SELECT logged_crm.concept_code_1,
		...
		FROM concept_relationship_manual logged_crm
		WHERE logged_crm.dev_schema_name = iTargetSchemaName
		AND EXISTS (
			--load only actual rels, because there could potentially be a situation when the relationship is in the log, but then (without the generic_update) it was removed in the dev-schema
			SELECT 1
		...
		WHERE ROW (base_crm.valid_start_date, base_crm.valid_end_date, base_crm.invalid_reason)
		IS DISTINCT FROM
		ROW (excluded.valid_start_date, excluded.valid_end_date, excluded.invalid_reason);
		...
		--if not - raise an error
		PERFORM FROM devv5.concept_manual local_cm
		LEFT JOIN devv5.base_concept_manual base_cm USING (
				concept_code,
				vocabulary_id
				)
		WHERE ROW (local_cm.concept_name, local_cm.domain_id, local_cm.concept_class_id, local_cm.standard_concept, local_cm.valid_start_date, local_cm.valid_end_date, local_cm.invalid_reason)
			...
			AND NOT EXISTS (
				SELECT 1
				FROM concept_manual logged_cm
				WHERE logged_cm.concept_code = local_cm.concept_code
		...
		FROM concept_manual logged_cm
		WHERE logged_cm.dev_schema_name = iTargetSchemaName
		AND EXISTS (
			--load only actual concepts, because there could potentially be a situation when the concept is in the log, but then (without the generic_update) it was removed in the dev-schema
			SELECT 1
		...
		SET concept_name = excluded.concept_name,
			...
			modified = COALESCE (excluded.modified, excluded.created),
			modified_by = COALESCE (excluded.modified_by, excluded.created_by)
		WHERE ROW (base_cm.concept_name, base_cm.domain_id, base_cm.concept_class_id, base_cm.standard_concept, base_cm.valid_start_date, base_cm.valid_end_date, base_cm.invalid_reason)
		IS DISTINCT FROM
		ROW (excluded.concept_name, excluded.domain_id, excluded.concept_class_id, excluded.standard_concept, excluded.valid_start_date, excluded.valid_end_date, excluded.invalid_reason);
		...
		--if not - raise an error
		PERFORM FROM devv5.concept_synonym_manual local_csm
		LEFT JOIN devv5.base_concept_synonym_manual base_csm USING (
				synonym_name,
				synonym_concept_code,
				synonym_vocabulary_id,
		...
		WHERE base_csm.synonym_concept_code IS NULL
			AND NOT EXISTS (
				SELECT 1
				FROM concept_synonym_manual logged_csm
				WHERE logged_csm.synonym_name = local_csm.synonym_name
		...
		SELECT logged_csm.synonym_name,
			...
			NULL --always NULL
		FROM concept_synonym_manual logged_csm
		WHERE logged_csm.dev_schema_name = iTargetSchemaName
			AND EXISTS (
		...
		DELETE
		FROM devv5.base_concept_synonym_manual base_csm
		USING devv5.vocabulary v
		WHERE v.vocabulary_id = base_csm.synonym_vocabulary_id
			AND v.latest_update IS NOT NULL
			AND NOT EXISTS (
				--missing
				SELECT 1
				FROM devv5.concept_synonym_manual local_csm
				WHERE local_csm.synonym_name = base_csm.synonym_name
			...
			AND EXISTS (
				...
				SELECT 1
				FROM devv5.concept_synonym_manual local_csm
				WHERE local_csm.synonym_concept_code = base_csm.synonym_concept_code
					AND local_csm.synonym_vocabulary_id = base_csm.synonym_vocabulary_id
				);
	END IF;
END;
